<template>
  <div class="button" v-bind:class="[color, size]" v-bind:style="{top: top + 'px', left: left + 'px'}">
    <i v-bind:class="{active: active}">
    </i>
    <em v-if="size === 's1'" v-bind:style="{transform: arrow + ' scale(1.2)'}"></em>
    <span v-bind:class="{position: position}">{{label}}</span>
  </div>
</template>
